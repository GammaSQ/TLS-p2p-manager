...
var tls = require('tls')
var net = require('net')
var onlinePackage = settings.useTLS? tls:net;
...
var PeerManager = exports.PeerManager = function PeerManager(options) {
  ...
  for (option in settings.defaultOptions){
    if(!(option in options)){options[option] = settings.defaultOptions[option];}
  }
...
PeerManager.prototype.openListener = function openListener() {
  ...
  var parentThis = this;
  var server = this.server = onlinePackage.createServer(settings.TLS_server_options, function(socket) {
    var p = new Peer(socket.remoteAddress, socket.remotePort, parentThis.options.magic);
...
PeerManager.prototype._parseHost = function _parseHost(element) {
  ...
  if(!element){
    ...
    socket = element;
  } else if (!fallback && element instanceof tls.TLSSocket){
    socket = element;
    host = socket.remoteAddress;
    port = socket.remotePort;
  } else if (fallback && element.__proto__ === target_proto){
    socket = element
...
PeerManager.prototype.addPool = function addPool(hosts) {
  ...
  for (var i = 0; i < hosts.length; i++) {
    var host = this._parseHost(hosts[i]);
    // No need to try and connect if host-parsing didn't work,
    // or if we already connected to it!
...
PeerManager.prototype._connect = function _connect(host, port) {
  ...
  p.on('connect', function(d) {
    //d.peer.host.host resolved to d.peer.socket.remoteAddress : d.peer.socket.remotePort
    if (parentThis.state = 'initiating') {
      parentThis.state = 'running'; // First peer to connect updates state
    }
  ...
  p.on('message', function(d) {
    ...
    parentThis.emit(d.command+'Message', {
      'peer':d.peer,
      'data':d.data
    });
  });
